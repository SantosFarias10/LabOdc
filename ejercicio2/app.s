...
main:
	...
	video:								//Parte donde se desplaza de derecha a izquierda
		bl fondo
		...
		bl LoopVideo					//Terminar parte donde se desplaza de derecha a izquierda

	videoRUNQEMU:						//Parte donde se escribe runQEMU y se ejecuta para reiniciar el programa
		bl runqemu

		bl InfLoop						//Termina el video
...
LoopVideo:								//Loop para hacer la primer parte del video
	bl 		delayLoop1
	add 	x7,x7,3
	subs	xzr,x7,130
	b.ge 	videoRUNQEMU
...
delayLoop1:								//Procedimiento para hacer un delay entre fotograma
	sub		sp,sp,8
	stur	x5,[sp,0]

	movz	x5,0x5ff,lsl 16
	Dl1:
		sub		x5,x5,1
		cbnz	x5,Dl1
...
delayLoop2:								//Procedimiento para hacer un delay entre fotograma mas lento
	sub		sp,sp,8
	stur	x5,[sp,0]

	movz	x5,0xfff,lsl 16
	Dl2:
		sub		x5,x5,1
		cbnz	x5,Dl2
...
ventana:								//Procedimiento para dibujar una ventana
	sub 	sp, sp, 8
	stur	lr, [sp, 0]

	movz 	x14, 0x1d2, lsl 0			/* marco */
	sub		x14, x14, x7
	movz	x15, 0x30, lsl 0
	movz	x12, 0x2f2, lsl 0
	sub 	x12, x12, x7
	movz 	x13, 0x130, lsl 0
	movz 	x10, 0xf5ee, lsl 0
	...
	sub 	x12, x12, 0x6
	sub 	x13, x13, 0x5
	movz 	x10, 0x0080, lsl 0
	movk	x10, 0x00, lsl 16
	bl 		rectangulo

	add		x14, x14, 0x30				/*montaña */
	add 	x15, x15, 0xb0
	movz 	x3, 0xb0, lsl 0
	movz 	x10, 0x6969, lsl 0
	movk 	x10, 0x69, lsl 16
	add 	x3, x3, 0x34
	bl 		triangulo

	add 	x14, x14, 0x6				/*sombra montaña */
	sub 	x3, x3, 0xb
	movz 	x10, 0x8080, lsl 0
	movk 	x10, 0x80, lsl 16
	bl 		triangulo

	sub 	x14, x14, 0x35				/*montaña 2 */
	sub 	x3, x3, 0x15
	movz 	x10, 0x6969, lsl 0
	movk 	x10, 0x69, lsl 16	
	bl		triangulo

	add 	x14, x14, 0x6				/*sombra */
	sub 	x3, x3, 0xc
	movz 	x10, 0x8080, lsl 0
	movk 	x10, 0x80, lsl 16	
	bl 		triangulo

	sub 	x14, x14, 0x7				/*pasto */
	movz 	x10, 0x6400, lsl 0
	movk 	x10, 0x00, lsl 16
	bl 		rectangulo

	add 	x14, x14, 0xfa				/*luna */
	sub 	x15, x15, 0x90
	movz 	x2,	0x10, lsl 0
	movz 	x10, 0xdcdc, lsl 0
...
lamparaTecho:							//Procedimiento para dibujar la lampara del techo
	sub 	sp, sp, 8
	stur	lr, [sp, 0]
...
cuadro:									//Procedimiento para dibujar un cuadro
	sub 	sp, sp, 8
	stur	lr, [sp, 0]
...
monitorEnNegro:						//Procedimiento para pintar la pantalla del monitor de negro
	sub 	sp,sp,8
	stur	lr,[sp,0]
...
pintarPixel:						//Procedimiento para pintar pixel, tomamos x10 = color, (x14,x15) = (X,Y)
	...
	stur 	x27,[sp,0]				
	subs	xzr, x14, xzr
	b.lt	skipPintarPixel
	subs	xzr,x14,SCREEN_WIDTH
	b.gt	skipPintarPixel
